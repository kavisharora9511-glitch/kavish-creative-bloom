...
import { ArrowDownRight, ArrowUpRight, GraduationCap, Instagram, Linkedin, Mail, Mic2, Sparkles, type LucideIcon } from "lucide-react";
...
const highlights: Array<{ icon: LucideIcon; label: string }> = [
  { icon: GraduationCap, label: "B.Tech Student" },
  { icon: Sparkles, label: "Creative Thinker" },
  { icon: ArrowUpRight, label: "Lifelong Learner" },
  { icon: Mic2, label: "Singer" },
];

const socials: Array<{ icon: LucideIcon; label: string }> = [
        ...
        <div className="section-shell grid items-start gap-12 lg:grid-cols-[.7fr_1.3fr]">
          ...
          <div>
            ...
            <div className="mt-10 grid gap-3 sm:grid-cols-2">
              {highlights.map(({ icon: Icon, label }) => <div key={label} className="glass-panel flex items-center gap-4 rounded-lg p-5 transition duration-300 hover:-translate-y-1"><span className="grid size-10 place-items-center rounded-full bg-secondary text-primary"><Icon className="size-4" /></span><span className="font-semibold">{label}</span></div>)}
            </div>
          </div>
        </div>
      </section>
